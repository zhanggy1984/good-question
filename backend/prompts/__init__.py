...
**与 cs/cc 的差别**：本目录 5 个模板里，3 个（chat_system / override_context /
rewrite_query）**带 {占位符}**，由调用方 .format() 填充；另 2 个是 RETRIEVE_TOOL_SCHEMA
的中文文案，无占位符、原样使用。故 load_prompt 只负责读取、**不做任何插值**——
插值时机与参数由调用方决定，不把各自的上下文知识塞进加载器。

这 3 个带占位符的模板经 AST 校验确认：除各自那一对花括号外正文再无 { }，
