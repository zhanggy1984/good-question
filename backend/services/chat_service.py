...
from services.retrieval_service import execute_retrieve_tool
from utils.exceptions import ForbiddenError, NotFoundError
from utils.query_normalizer import clean_query, normalize_query
...
def stream_chat(session_id: int, user_content: str):
    ...
    try:
        ...
        if not history:
            ...
            cache_question = normalize_query(user_content)
        ...
        tool_calls = None
        try:
            for ev in llm_stream_round1_with_retry(messages, tools=[RETRIEVE_TOOL_SCHEMA]):
                llm_rounds = 1
                if ev["type"] == "usage":
        ...
        if decided_retrieve:
            ...
            if result and result["source_count"] > 0:
                ...
                try:
                    for ev in llm_stream_chat(messages):  # 不带 tools，防再循环
                        llm_rounds = 2
                        if ev["type"] == "usage":
                            _merge_usage(ev["usage"])
                        elif ev["type"] == "reasoning":
                ...
                if intent != "smalltalk" and not non_doc_question:
                    ...
                    try:
                        for ev in llm_stream_chat(messages):
                            llm_rounds = 2
                            if ev["type"] == "usage":
                                _merge_usage(ev["usage"])
                            elif ev["type"] == "reasoning":
            ...
            query = clean_query(user_content, user_content)
            try:
